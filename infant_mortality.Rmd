...
Babies with a birthweight of less than 5 pounds, 8 ounces (2,500 grams) are considered to have a low birth weight. Low birth weight is primarily caused by premature birth. Babies with a low birth weight can face multiple challenges including having trouble keeping their bodies at the right temperature, having trouble breathing, nad difficulty feeding and gaining weight.
...
low_birth <- low_birth %>%
  ggplot(aes(x = as.character(`Birth Year`), y = Count, group = `Low Birth Weight Indicator`, fill = `Low Birth Weight Indicator`)) +
           geom_bar(stat = "identity", position = "dodge") +
  geom_text(aes(label=Count), vjust=1.5, color="white", size=5, position = position_dodge(width=.9)) +
  theme_minimal() +
  theme(
  axis.title.x = element_blank()
  #axis.text.x = element_blank(),
...
Tobacco use can be harmful to a developing baby and the mother. Maternal smoking is the percent of mothers who smoked during the second or third trimester

```{r}
maternal_smoking <- birth %>%
  mutate(smoked = ifelse(`Smoked Second Trimester` == "Y", "Y", "N"))

  
  filter(`Smoked Second Trimester` == "Y" | `Smoked Last Trimester` == "Y") %>%
  filter(`Birth Year` > 2011 & `Birth Year` < 2017) %>%
